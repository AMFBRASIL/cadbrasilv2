...
export function RenovarSicafPage() {
  return (
    <PageShell>
          ...
          <div className="grid lg:grid-cols-[1fr_340px] gap-10 lg:gap-14 items-center">
            <div className="text-white">
              <span className="inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-widest text-white/70 border border-white/20 rounded-full px-3 py-1">
                ...
                Campanha renovação SICAF · 2026
              </span>

              <h1 className="mt-5 text-3xl sm:text-4xl lg:text-[2.75rem] font-bold leading-[1.1] tracking-tight text-balance">
                Renove o SICAF antes do vencimento e{" "}
                <span className="text-[oklch(0.82_0.08_250)]">continue apto a licitar</span>
              </h1>

              <p className="guide-hero-lead mt-5 text-base sm:text-lg text-white/90 leading-relaxed max-w-2xl">
                Certidões vencidas são a causa mais evitável de desclassificação. A{" "}
                <strong className="text-white font-semibold">CADBRASIL</strong> monitora prazos, renova documentos e
                atualiza o Compras.gov.br para manter seu <strong className="text-white font-semibold">CRC Regular</strong>.
              </p>

              <p className="guide-summary mt-3 text-sm text-white/65 max-w-xl leading-relaxed">
                Renovação não é anual fixa — é ciclo contínuo por certidão. Comece agora e evite urgência na véspera do
                pregão.
              </p>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <CadastroLink className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-white text-[oklch(0.28_0.09_260)] font-semibold shadow-lg hover:scale-[1.02] transition">
                  Renovar meu SICAF agora
                  <ArrowRight className="h-4 w-4" />
                </CadastroLink>
                <WhatsAppLink
                  pageLabel="Campanha Renovar SICAF"
                  intent="Quero renovar as certidões e manter meu SICAF regular com a CADBRASIL."
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:py-10">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground mb-6">
          <nav aria-label="Breadcrumb">
            <ol className="flex flex-wrap items-center gap-2">
              ...
              <li aria-hidden>/</li>
              <li>
                <Link to="/renovacao-sicaf" className="hover:text-brand">
                  Renovação SICAF
                </Link>
              </li>
              <li aria-hidden>/</li>
              <li className="text-foreground font-medium">Renovar SICAF</li>
            </ol>
          </nav>
        ...
        <div className="grid lg:grid-cols-[260px_minmax(0,1fr)] gap-10 xl:gap-14 items-start">
          <aside className="hidden lg:block">
            <nav
              aria-label="Índice renovação SICAF"
          ...
          <article className="min-w-0">
            <section id="resposta-rapida" className="scroll-mt-32 mb-14">
              <div className="rounded-2xl border border-brand/25 bg-brand/5 p-6 sm:p-8">
                ...
                <p className="guide-quick-answer text-base text-foreground leading-relaxed">{renovarSicafMeta.quickAnswer}</p>
              </div>
            </section>
            ...
            <section id="o-que-e" className="scroll-mt-32 mb-14">
              <h2 className="text-2xl sm:text-3xl font-bold mb-4 flex items-center gap-2">
                <RefreshCw className="h-7 w-7 text-brand shrink-0" />
                O que é renovação do SICAF?
              </h2>
              <div className="space-y-4">
                {oQueERenovacao.map((p) => (
            ...
            <section id="quando-renovar" className="scroll-mt-32 mb-14">
              <h2 className="text-2xl sm:text-3xl font-bold mb-4 flex items-center gap-2">
                <Bell className="h-7 w-7 text-brand shrink-0" />
                Quando renovar?
              </h2>
              <div className="grid sm:grid-cols-2 gap-4">
                {quandoRenovar.map((item) => (
            ...
            <section id="riscos" className="scroll-mt-32 mb-14 rounded-3xl border border-destructive/20 bg-destructive/5 p-6 sm:p-8">
              <h2 className="text-2xl sm:text-3xl font-bold mb-4 flex items-center gap-2">
                <FileWarning className="h-7 w-7 text-destructive shrink-0" />
                Riscos de não renovar
              </h2>
              <ul className="space-y-2">
                {riscosNaoRenovar.map((r) => (
                  <li key={r} className="flex gap-2 text-sm text-muted-foreground bg-card border border-border rounded-xl p-4">
            ...
            <section id="certidoes" className="scroll-mt-32 mb-14">
              <h2 className="text-2xl sm:text-3xl font-bold mb-4">Certidões e prazos típicos</h2>
                  ...
                  <tbody>
                    {certidoesPrazos.map((row) => (
                      <tr key={row.doc} className="border-t border-border">
                        ...
                        <td className="p-4 text-muted-foreground text-xs">{row.tipo}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <section id="como-renovar" className="scroll-mt-32 mb-14">
              <h2 className="text-2xl sm:text-3xl font-bold mb-2 flex items-center gap-2">
                <ListChecks className="h-7 w-7 text-brand shrink-0" />
                Como renovar — passo a passo
              </h2>
              <p className="text-muted-foreground mb-6">Fluxo prático no Compras.gov.br (schema HowTo para Google e IAs).</p>
              <ol className="space-y-3">
                {passosRenovacao.map((step, index) => (
                  <li
                    key={step.n}
                    id={`passo-${index + 1}`}
            ...
            <section id="nivel-iv" className="scroll-mt-32 mb-14 rounded-3xl border-2 border-amber-500/30 bg-amber-500/5 p-6 sm:p-8">
              <h2 className="text-2xl sm:text-3xl font-bold mb-2 flex items-center gap-2">
                <Trash2 className="h-7 w-7 text-amber-700 shrink-0" />
                {nivelIvDestaque.titulo}
              </h2>
              <div className="mt-6 grid md:grid-cols-3 gap-4">
                {nivelIvDestaque.passos.map((p, i) => (
                  <div key={p} className="rounded-2xl border border-amber-500/25 bg-card p-5 text-center shadow-card">
                    <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-amber-600 text-white font-bold">
                      {i + 1}
                    </div>
              ...
              <ul className="mt-6 grid sm:grid-cols-2 gap-2">
                {nivelIvDestaque.checklist.map((c) => (
              ...
              <Link to="/sicaf-niveis" className="inline-flex items-center gap-2 mt-4 text-sm text-brand font-semibold hover:underline">
                Ver manual ilustrado dos níveis
                <ArrowRight className="h-4 w-4" />
              </Link>
            </section>

            <section id="cadbrasil" className="scroll-mt-32 mb-14">
              <h2 className="text-2xl sm:text-3xl font-bold mb-6">Renovação com a CADBRASIL</h2>
              <div className="grid sm:grid-cols-2 gap-4">
                {renovacaoCadbrasil.map((s) => (
            ...
            <section id="comparativo" className="scroll-mt-32 mb-14">
              <h2 className="text-2xl sm:text-3xl font-bold mb-4">Sozinho vs CADBRASIL</h2>
              <div className="overflow-x-auto rounded-2xl border border-border bg-card shadow-card">
              ...
              <p className="mt-2 text-xs text-muted-foreground">* Com documentação pronta e sem pendências estruturais.</p>
            </section>

            <section id="faq" className="scroll-mt-32 mb-14">
              <h2 className="text-2xl sm:text-3xl font-bold mb-6">Perguntas frequentes</h2>
              <div className="space-y-3">
                {renovarSicafFaqs.map((faq) => (
            ...
            <section id="cta" className="scroll-mt-32 mb-14">
              <div className="rounded-3xl border border-brand/25 bg-gradient-to-br from-brand/10 via-card to-card p-8 sm:p-10 text-center shadow-card">
                <h2 className="text-2xl sm:text-3xl font-bold">Pronto para renovar seu SICAF?</h2>
                <p className="mt-3 text-muted-foreground max-w-2xl mx-auto">
                  Monitore vencimentos, atualize certidões e mantenha o CRC Regular — com especialistas CADBRASIL.
                </p>
                <div className="mt-6 flex flex-wrap justify-center gap-3">
                  ...
                  <WhatsAppLink
                    pageLabel="Campanha Renovar SICAF"
                    intent="Quero renovar meu SICAF com urgência."
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-border bg-card font-semibold hover:border-brand/40 transition"
                  >
                    <MessageCircle className="h-4 w-4" />
                    WhatsApp
